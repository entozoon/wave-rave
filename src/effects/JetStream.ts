...
export default class {
  ...
  constructor({ hero }) {
    ...
    //
    //
    let renderTexture = Engine.renderer.pixi.RenderTexture.create({
      width: 800,
      height: 600,
    });
    let swirl = Engine.renderer.pixi.Sprite.from("emitters/jet.png");
    swirl.position.x = 100;
    swirl.position.y = 100;
    Engine.renderer.layers.water.renderer.render(swirl, { renderTexture });
  }
  update() {
    //   if (Math.random() > 0.95) {
    //     let swirl = Engine.renderer.pixi.Sprite.from("emitters/jet.png");
